...
def talk_to_me(update, context):
    ...
    nfm=ephem.next_full_moon(user_message)
    print(nfm)
...
def moon_planet(update, context):
    update.message.reply_text("Когда ближайшее полнолуние?Введите дату")

def main():
    mybot = Updater(settings.API_KEY, use_context=True, request_kwargs=PROXY)

    dp = mybot.dispatcher
    dp.add_handler(CommandHandler("moonplanet",moon_planet))
